...
function BuscarAmigosScreen() {
  ...
  useEffect(() => {
    async function obtenerUsuarios() {
      try {
        const respuesta = await fetch('http://192.168.1.234:8080/usuarios');
  ...
  async function seguirUsuario(usuarioId) {
    try {
      const respuesta = await fetch(`http://192.168.1.234:8080/amigosMovil/seguir/${usuarioId}`, {
        method: 'POST',
...
function MisAmigosScreen() {
  ...
  useEffect(() => {
    async function obtenerAmigos() {
      try {
        const respuesta = await fetch('http://192.168.1.234:8080/misamigos');
        const datos = await respuesta.json();
        setUsuarios(datos);
      } catch (error) {
        console.error(error);
  ...
  async function dejarSeguir(usuarioId) {
    try {
      const respuesta = await fetch(`http://192.168.1.234:8080/amigosMovil/dejar/${usuarioId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        // Puedes agregar aquí el token de autenticación si es necesario
      });
...
function RankingScreen() {
  ...
  useEffect(() => {
    async function obtenerUsuarios() {
      try {
        const respuesta = await fetch('http://192.168.1.234:8080/rankingAmigos');
        const datos = await respuesta.json();
        setUsuarios(datos);
      } catch (error) {
        console.error(error);
